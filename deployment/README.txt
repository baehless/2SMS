...
- The current system supports only a single Scraper instance
- It is assumed that both Manager and Scraper will be installed in the same local network behind a NAT
...
- Deployment of other external components (e.g. alertmanager, blackbox_exporter, node_exporter, ...) are not covered here
...
# Procedure to deploy from scratch
    1) Run `install_manager.sh` (change IP accordingly to the setting) to install and run the Manager.
